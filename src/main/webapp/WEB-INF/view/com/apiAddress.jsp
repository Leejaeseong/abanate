...
function init(){
	...
	var confmKeyPc 		= "<%=request.getAttribute( "SETTING_API_ADDRESS_KEY_PC" )%>";		// PC용 승인키
	var confmKeyMobile 	= "<%=request.getAttribute( "SETTING_API_ADDRESS_KEY_MOBILE" )%>";	// Mobile용 승인키
	// resultType항목 추가(2016.10.06)
	var resultType = "4"; // 도로명주소 검색결과 화면 출력유형, 1 : 도로명, 2 : 도로명+지번, 3 : 도로명+상세건물명, 4 : 도로명+지번+상세건물명
	var inputYn= "<%=inputYn%>";
	if(inputYn != "Y"){
		document.form.confmKey.value = confmKeyMobile;
		...
		/** API 서비스 제공항목 확대 (2017.02) **/
		opener.addressCallBack("<%=roadFullAddr%>","<%=roadAddrPart1%>","<%=addrDetail%>", "<%=roadAddrPart2%>","<%=engAddr%>"
